...
public class MapleEquipOnlyId {
    ...
    public long initOnlyId() {
        ...
        try (PreparedStatement ps = DatabaseConnection.getInstance().getDataSource().getConnection().prepareStatement("SELECT MAX(equipOnlyId) FROM inventoryitems WHERE equipOnlyId > 0"); ResultSet rs = ps.executeQuery()) {
            if (rs.next()) {
                ret = rs.getLong(1) + 1;
            }
            ps.close();

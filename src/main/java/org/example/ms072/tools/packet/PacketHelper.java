...
import  org.example.ms072.handling.Buffstat;
import  org.example.ms072.handling.world.MapleCharacterLook;
import java.util.*;
...
import  org.example.ms072.server.stores.AbstractPlayerStore;
import  org.example.ms072.server.stores.IMaplePlayerShop;
import org.example.ms072.tools.BitTools;
...
//    }
    ...
    public static void addCharLook(MaplePacketLittleEndianWriter mplew, MapleCharacterLook chr, boolean mega, boolean second) {
        ...
        for (Entry entry : myEquip.entrySet()) {
            mplew.write(((Byte) entry.getKey()));
            mplew.writeInt(((Integer) entry.getValue()));
        }
        mplew.write(255);

        for (Entry entry : maskedEquip.entrySet()) {
            mplew.write(((Byte) entry.getKey()));
            mplew.writeInt(((Integer) entry.getValue()));
        }
        mplew.write(255);

...
//        }
    ...
    public static byte[] sendWishList(MapleCharacter chr, boolean update) {
        ...
        MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
        mplew.writeShort(SendPacketOpcode.CS_OPERATION.getValue());
        mplew.write(Operation_Code + (update ? 13 : 9));
        ...
        try {
            ...
            try (PreparedStatement ps = con.prepareStatement("SELECT sn FROM wishlist WHERE characterid = ? LIMIT 10")) {
            ...
            System.out.println("获取商城购物车数据错误:" + se);
        }
        while (i > 0) {
            mplew.writeInt(0);
            i--;
     ...
     * 商城送礼物
     */
    public static byte[] 商城送礼物(int itemid, int quantity, String receiver) {

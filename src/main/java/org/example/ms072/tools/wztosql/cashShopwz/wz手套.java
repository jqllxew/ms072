...
public class wz手套 {
    ...
    public static final CashModInfo getModInfo(int sn) {
        ...
        try (Connection con = DatabaseConnection.getInstance().getDataSource().getConnection(); PreparedStatement ps = con.prepareStatement("SELECT * FROM wz_207_cashshop_装备_手套 WHERE serial = ?")) {
            ps.setInt(1, sn);
    ...
    public static void main(String[] args) {
        ...
        CashItemFactory.getInstance().initialize_207();
        ...
        for (Map<String, Integer> field : itemConmes.values()) {
            try {
                ...
                try (Connection con = DatabaseConnection.getInstance().getDataSource().getConnection(); PreparedStatement ps = con.prepareStatement("INSERT INTO wz_207_cashshop_装备_手套 (serial, showup,itemid,priority,period,gender,count,meso,discount_price,mark, unk_1, unk_2, unk_3) VALUES ( ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)")) {
                    ps.setInt(1, sn);
                    ps.setInt(2, 1);
                    ps.setInt(3, itemId);
                    ps.setInt(4, 0);

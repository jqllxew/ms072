...
import  org.example.ms072.client.SkillEntry;
...
import  org.example.ms072.custom.SearchGenerator;
import  org.example.ms072.custom.CustomPlayerRankings;
...
import  org.example.ms072.handling.channel.handler.HiredMerchantHandler;
import  org.example.ms072.handling.channel.handler.InventoryHandler;
import  org.example.ms072.handling.channel.handler.PlayersHandler;
import  org.example.ms072.handling.login.LoginInformationProvider;
...
import  org.example.ms072.handling.world.exped.ExpeditionType;
...
import  org.example.ms072.server.MapleCarnivalChallenge;
import  org.example.ms072.server.MapleCarnivalParty;
import  org.example.ms072.server.MapleInventoryManipulator;
import  org.example.ms072.server.MapleItemInformationProvider;
import  org.example.ms072.server.MapleSlideMenu;
import  org.example.ms072.server.MapleSquad;
import  org.example.ms072.server.MapleStatEffect;
import  org.example.ms072.server.Randomizer;
import  org.example.ms072.server.MapleSlideMenu.SlideMenu0;
import  org.example.ms072.server.MapleSlideMenu.SlideMenu1;
import  org.example.ms072.server.MapleSlideMenu.SlideMenu2;
import  org.example.ms072.server.MapleSlideMenu.SlideMenu3;
import  org.example.ms072.server.MapleSlideMenu.SlideMenu4;
import  org.example.ms072.server.MapleSlideMenu.SlideMenu5;
import  org.example.ms072.server.MerchItemPackage;
import  org.example.ms072.server.SpeedRunner;
import  org.example.ms072.server.StructItemOption;
import  org.example.ms072.server.Timer;
import  org.example.ms072.server.Timer.CloneTimer;
import  org.example.ms072.server.life.*;
import  org.example.ms072.server.maps.Event_DojoAgent;
import  org.example.ms072.server.maps.Event_PyramidSubway;
import  org.example.ms072.server.maps.MapleMap;
import  org.example.ms072.server.quest.MapleQuest;
import  org.example.ms072.server.shops.MapleShopFactory;
import  org.example.ms072.server.stores.IMaplePlayerShop;
...
import  org.example.ms072.tools.packet.CField.UIPacket;
import  org.example.ms072.tools.packet.CWvsContext;
import  org.example.ms072.tools.packet.CWvsContext.GuildPacket;
import  org.example.ms072.tools.packet.CWvsContext.InfoPacket;
import  org.example.ms072.tools.packet.PlayerShopPacket;
...
public class NPCConversationManager extends AbstractPlayerInteraction {
    ...
    public NPCConversationManager(MapleClient c, int npc, int questid, String npcscript, byte type, Invocable iv, int wh) {
    ...
    public String getDimensionalMirror(MapleCharacter character) {
        return SlideMenu0.getSelectionInfo(character, id);
    }
    ...
    public void sendNextPrevS(String text, byte type, int idd, int npcid) {
        ...
        c.sendPacket(NPCPacket.getNPCTalk(npcid, (byte) 0, text, "01 01", type, idd));
        lastMsg = 0;
    }
...
//    }
    ...
    public void sendYesNo(String text, int id) {
        ...
        c.sendPacket(NPCPacket.getNPCTalk(id, (byte) 1, text, "", (byte) 0));
        lastMsg = 1;
    }
    ...
    public void sendYesNoS(String text, byte type, int idd) {
        ...
        c.sendPacket(NPCPacket.getNPCTalk(id, (byte) 1, text, "", type, idd));
        lastMsg = 1;
    }
    ...
    public void askAcceptDecline(String text, int id) {
        ...
        c.sendPacket(NPCPacket.getNPCTalk(id, lastMsg, text, "", (byte) 0));
    }
    ...
    public void askAcceptDeclineNoESC(String text, int id) {
        ...
        lastMsg = (byte) 0x0C;
        c.sendPacket(NPCPacket.getNPCTalk(id, lastMsg, text, "", (byte) 1));
    }
    ...
    public void sendSimple(String text, int id) {
        ...
        c.sendPacket(NPCPacket.getNPCTalk(id, (byte) 4, text, "", (byte) 0));
        lastMsg = 4;
    }
    ...
    public void sendSimpleS(String text, byte type, int idd) {
        ...
        c.sendPacket(NPCPacket.getNPCTalk(id, (byte) 5, text, "", type, idd));
        lastMsg = 4;
    }
    ...
    public void sendStyle(String text, int caid, int styles[]) {
        ...
        c.sendPacket(NPCPacket.getNPCTalkStyle(id, text, caid, styles));
        lastMsg = 7;
    }
    ...
    public void openShop(int id) {
        if (World.isShutDown) {
            c.getPlayer().dropMessage(1, "服务器正在关闭，现在无法使用商店！\r\n请立即下线，否则后果自负！");
            c.sendPacket(CWvsContext.enableActions());
            return;
        }
    ...
    public void openShopNPC(int id) {
        if (World.isShutDown) {
            c.getPlayer().dropMessage(1, "服务器正在关闭，现在无法使用商店NPC！\r\n请立即下线，否则后果自负！");
            c.sendPacket(CWvsContext.enableActions());
            return;
        }
    ...
    public int gainGachaponItem2(int id, int quantity, String msg, int 概率) {
        return gainGachaponItem2(id, quantity, msg, 概率, 0);
    }
    ...
    public int gainGachaponItem2(int id, int quantity, String msg, int 概率, int 类型) {
        try {
            ...
            Item item = MapleInventoryManipulator.addbyId_Gachapon(c, id, (short) quantity);
            ...
            if (概率 > 0 && 类型 > 0) {
                switch (类型) {
                    case 1, 2, 3, 4, 5, 6
                            -> World.Broadcast.broadcastMessage(CWvsContext.getGachaponMega2("[" + msg + "] " + c.getPlayer().getName(), " : 从抽奖中获得，大家恭喜他（她）吧！！！", item, (byte) 0, getPlayer().getClient().getChannel()));
                    default -> World.Broadcast.broadcastMessage(CWvsContext.getGachaponMega2("[" + msg + "] " + c.getPlayer().getName(), "", item, (byte) 0, getPlayer().getClient().getChannel()));
                }
            } else {
                World.Broadcast.broadcastMessage(CWvsContext.getGachaponMega2("[" + msg + "] " + c.getPlayer().getName(), " : 从抽奖中获得，大家恭喜他（她）吧！！！", item, (byte) 0, getPlayer().getClient().getChannel()));
            }
    ...
    public int gainGachaponItem(int id, int quantity, final String msg) {
        try {
            ...
            if (rareness > 0) {
                World.Broadcast.broadcastMessage(CWvsContext.getGachaponMega(c.getPlayer().getName(), " : 从抽奖中获得", item, rareness, msg));
            } else {
                World.Broadcast.broadcastMessage(CWvsContext.getGachaponMega(c.getPlayer().getName(), " : 从抽奖中获得", item, rareness, msg));
            }
...
//public void teachSkill(int id, int skillevel, byte masterlevel, long expiration) {
    ...
    public void openMerchantItemStore() {
        ...
        if (merch) {
            ...
            c.sendPacket(PlayerShopPacket.merchItemStore((byte) 0x25, 0, 0));
            c.getPlayer().setConversation(0);
        } else {
    ...
    public void sendQuestWindow() {
        c.sendPacket(UIPacket.openUI(6));
    }
    ...
    public void sendCassandrasCollectionWindow() {
        c.sendPacket(UIPacket.openUI(127));
    }

    public void sendLuckyLuckyMonstoryWindow() {
        c.sendPacket(UIPacket.openUI(120));
    }
    ...
    public final void doWeddingEffect(final Object ch) {
        ...
        getMap().broadcastMessage(CWvsContext.yellowChat(player.getName() + ", 你愿意娶 " + chr.getName() + " 为妻吗？无论她将来是富有还是贫穷、或无论她将来身体健康或不适，你都愿意和她永远在一起吗？"));
        CloneTimer.getInstance().schedule(new Runnable() {
            ...
            public void run() {
                if (chr == null || player == null) {
                    ...
                    chr.getMap().broadcastMessage(CWvsContext.yellowChat(chr.getName() + ", 你愿意嫁给 " + player.getName() + " 吗？无论他将来是富有还是贫穷、或无论他将来身体健康或不适，你都愿意和他永远在一起吗？"));
                }
            }
        }, 10000);
        CloneTimer.getInstance().schedule(new Runnable() {
            ...
            public void run() {
                if (chr == null || player == null) {
                    ...
                    chr.getClient().sendPacket(CField.sendMarrageEffect());
                    player.getClient().sendPacket(CField.sendMarrageEffect());
                    ...
                    World.Broadcast.broadcastMessage(CWvsContext.yellowChat("好，我以圣灵、圣父、圣子的名义宣布：" + player.getName() + " 和 " + chr.getName() + "结为夫妻。 希望你们在 " + chr.getClient().getChannelServer().getServerName() + " 游戏中玩的愉快!"));
    ...
    public void mihileNeinheartDisappear() {
        try {
            ...
            c.getPlayer().getClient().sendPacket(UIPacket.getDirectionInfo((byte) 1, 2000));
            Thread.sleep(2000);
    ...
    public void mihileMove913070001() {
        try {
            c.getPlayer().getClient().sendPacket(UIPacket.getDirectionInfo((byte) 3, 2));
            c.getPlayer().getClient().sendPacket(UIPacket.getDirectionInfo((byte) 1, 800));
            Thread.sleep(800);
        } catch (InterruptedException e) {
        }
        c.sendPacket(UIPacket.IntroEnableUI(0));
    ...
    public void mihileMove913070050() {
        try {
            c.getPlayer().getClient().sendPacket(UIPacket.getDirectionInfo((byte) 3, 2));
            c.getPlayer().getClient().sendPacket(UIPacket.getDirectionInfo((byte) 1, 6000));
            Thread.sleep(5000);
            c.getPlayer().getClient().sendPacket(UIPacket.getDirectionInfo((byte) 3, 0));
    ...
    public void enter_931060110() {
        try {
            ...
            c.sendPacket(UIPacket.getDirectionInfo((byte) 1, 900));
            Thread.sleep(900);
            ...
            c.sendPacket(UIPacket.getDirectionInfo((byte) 1, 720));
            Thread.sleep(720);
    ...
    public void enter_931060120() {
        try {
            ...
            c.sendPacket(UIPacket.getDirectionInfo((byte) 1, 1650));
            Thread.sleep(1650);
            ...
            c.sendPacket(UIPacket.getDirectionInfo((byte) 1, 540));
            Thread.sleep(540);
            ...
            c.sendPacket(UIPacket.getDirectionInfo((byte) 1, 1380));
            Thread.sleep(1380);
    ...
    public void showJettWanted() {
        try {
            ...
            c.sendPacket(UIPacket.getDirectionFacialExpression(5, 3000));
    ...
    public void np_tuto_0_2() {
        try {
            ...
            c.sendPacket(UIPacket.getDirectionInfo(1, 10));
            Thread.sleep(10);
            c.sendPacket(UIPacket.getDirectionFacialExpression(5, 3000));
            c.sendPacket(UIPacket.getDirectionInfo("Effect/DirectionNewPirate.img/newPirate/balloonMsg1/1", 2000, 0, -80, 0, 0));
            c.sendPacket(UIPacket.getDirectionInfo(1, 1000));
    ...
    public void showBeastTamerTutScene1() {
        try {
            ...
            c.sendPacket(UIPacket.getDirectionInfo(1, 800));
            Thread.sleep(800);
            c.sendPacket(UIPacket.getDirectionInfoNew((byte) 0, 1000, 700, 0));
            ...
            c.sendPacket(UIPacket.getDirectionInfo(1, 500));
            Thread.sleep(1500);
            c.sendPacket(UIPacket.getDirectionFacialExpression(4, 5000));
            Thread.sleep(3000);
        } catch (InterruptedException ex) {
        }
        NPCScriptManager.getInstance().dispose(c);
    ...
    public void openBeans() {//打开豆豆机界面
        c.sendPacket(CField.openBeans(c.getPlayer()));
    }

    public void 喇叭(int lx, String msg) {
        switch (lx) {
            case 1://白色背景双红心公告
                World.Broadcast.broadcastSmega(CWvsContext.serverNotice(1, "[全服公告] : " + msg));
                break;
            case 2://蓝色文字公告
                World.Broadcast.broadcastSmega(CWvsContext.serverNotice(2, "[全服公告] : " + msg));
                break;
            case 3://红色背景喇叭
                World.Broadcast.broadcastSmega(CWvsContext.broadcastMessage(3, this.c.getChannel(), "[全服公告] : " + msg, true));
                break;
            case 4://黄色背景喇叭
                World.Broadcast.broadcastSmega(CWvsContext.broadcastMessage(9, c.getChannel(), "[全服公告] : " + msg, false));
                break;
            case 5://白色背景喇叭
                World.Broadcast.broadcastSmega(CWvsContext.serverNotice(11, "[全服公告] : " + msg));
                break;
        }
    }
    ...
    public void 等级排行榜() {
        MapleGuild.displayLevelRanks(getClient(), npc);
    }

    public void 金币排行榜() {
        MapleGuild.displayMesoRanks(getClient(), npc);
    }

    public void 人气排行榜() {
        MapleGuild.displayFameRanks(getClient(), npc);
    }
}

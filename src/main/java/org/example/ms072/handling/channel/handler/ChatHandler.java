...
import  org.example.ms072.constants.ServerConstants.CommandType;
import org.example.ms072.handling.channel.ChannelServer;
import  org.example.ms072.handling.world.MapleMessenger;
    ...
    public static void GeneralChat(final String text, final byte unk, final MapleClient c, final MapleCharacter chr) {
        if (text.length() > 0 && chr != null && chr.getMap() != null && !CommandProcessor.processCommand(c, text, CommandType.NORMAL)) {
            ...
            if (chr.getCanTalk() || chr.isStaff()) {
                ...
                c.sendPacket(CWvsContext.broadcastMsg(6, "你已经被禁言了, 所以无法说话。"));
            }
        }
    }
            ...
            case 5: { // Find
                ...
                if (player != null) {
                    if (!player.isIntern() || c.getPlayer().isIntern() && player.isIntern()) {
                        ...
                        c.sendPacket(CField.getFindReplyWithMap(player.getName(), player.getMap().getId(), mode == 68));//68
                    } else {

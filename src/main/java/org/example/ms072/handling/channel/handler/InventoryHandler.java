...
import  org.example.ms072.client.InnerAbillity;
import  org.example.ms072.client.InnerSkillValueHolder;
...
import  org.example.ms072.client.MonsterFamiliar;
import  org.example.ms072.client.PlayerStats;
...
import  org.example.ms072.client.inventory.Equip.ScrollResult;
...
import  org.example.ms072.client.inventory.MapleMount;
import  org.example.ms072.client.inventory.MaplePet;
import  org.example.ms072.client.inventory.MaplePet.PetFlag;
...
import  org.example.ms072.server.PredictCardFactory;
import  org.example.ms072.server.RandomRewards;
...
import  org.example.ms072.server.maps.MapleLove;
import  org.example.ms072.server.maps.MapleMap;
import  org.example.ms072.server.maps.MapleMapItem;
import  org.example.ms072.server.maps.MapleMapObject;
import  org.example.ms072.server.maps.MapleMapObjectType;
import  org.example.ms072.server.maps.MapleMist;
...
import  org.example.ms072.tools.packet.CWvsContext.InventoryPacket;
import  org.example.ms072.tools.packet.CSPacket;
import  org.example.ms072.tools.packet.MobPacket;
import  org.example.ms072.tools.packet.PetPacket;
...
public class InventoryHandler {
    ...
    public static final void ItemSort(final LittleEndianAccessor slea, final MapleClient c) {
        ...
        c.sendPacket(CWvsContext.enableActions());
        c.sendPacket(CWvsContext.serverNotice(1, "道具集合完毕"));
    }
    ...
    public static final void ItemGather(final LittleEndianAccessor slea, final MapleClient c) {
        ...
        itemMap.clear();
        sortedItems.clear();
        c.sendPacket(CWvsContext.serverNotice(1, "以种类排序完毕。"));
    }
                ...
                if (rewards != null && rewards.getLeft() > 0) {
                    ...
                    if (itemId >= 2028154 && itemId <= 2028156
                        ...
                        if (unseal) {
                            ...
                            MapleInventoryManipulator.addbyItem(c, item);
                            c.sendPacket(CField.unsealBox(item.getItemId()));
                            c.sendPacket(EffectPacket.showRewardItemAnimation(itemId, ""));
                        } else {
                            c.sendPacket(CField.sendSealedBox(slot, itemId, items));
                        }
                        return true;
                    }
                    switch (itemId) {
...
//        }
        ...
        } else if (scrollSuccess == ScrollResult.FAIL && scrolled.getUpgradeSlots() < oldSlots && c.getPlayer().getInventory(MapleInventoryType.CASH).findById(5640000) != null) {
        ...
        switch (itemId) {
            ...
            case 5500000: { // Magic Hourglass 1 day
                ...
                if (item != null && !GameConstants.isAccessory(item.getItemId()) && item.getExpiration() > -1 && !ii.isCash(item.getItemId()) && System.currentTimeMillis() + (100 * 24 * 60 * 60 * 1000L) > item.getExpiration() + (days * 24 * 60 * 60 * 1000L)) {
                    ...
                    if (change) {
                        ...
                        used = true;
                        c.getPlayer().dropMessage(1, "使用成功！有效期延长一天！");
                    } else {
            ...
            case 5320000:
              ...
              /*  if ((Card == null) || (Comment == null)) {
                    ...
                    c.getPlayer().dropMessage(1, "请填写自己的角色昵称。");
                    c.sendPacket(CWvsContext.enableActions());
                    return;
                } else if (victim == null) {
                    c.getPlayer().dropMessage(1, "对方不在线上。");
                    c.sendPacket(CWvsContext.enableActions());
                    return;
                } else if (c.getPlayer().getGMLevel() < victim.getGMLevel()) {
                    c.getPlayer().dropMessage(1, "无法对此人使用。");
                    c.sendPacket(CWvsContext.enableActions());
                    return;
                }*/
               ...
               // c.getPlayer().getMap().broadcastMessage(CField.startMapEffect("", 0, false)); // 清除當前地圖速配結果
                c.sendPacket(CWvsContext.showPredictCard(name, otherName, love, cardId, Comment.effectType));
               // c.getPlayer().getMap().broadcastMessage(CField.startMapEffect(name + "和" + otherName + "的速配程度 " + Comment.worldmsg0, 5120026, true));
                used = true;
                break;
            case 5370001:
            ...
            case 5201000:// - 豆豆箱 - 有2000个豆豆的箱子。
                ...
                cc = true;
                break;
            case 5201001:// - 豆豆箱(500个) - 有500个豆豆的箱子。在玩具城、昭和村、废都的豆豆屋中可以使用，使用前需先双击充进角色背包中。
                ...
                cc = true;
                break;
            case 5201002:// - 豆豆箱(3000个) - 有3000个豆豆的箱子。在玩具城、昭和村、废都的豆豆屋中可以使用，使用前需先双击充进角色背包中。
...
//                    System.err.println("Return 12");
   ...
   /* private static boolean getIncubatedItems(MapleClient c, int itemId) {
        ...
        c.sendPacket(NPCPacket.getNPCTalk(1090000, (byte) 0, "您已获得以下物品:\r\n#i" + id1 + "##z" + id1 + "#\r\n#i" + id2 + "##z" + id2 + "#", "00 00", (byte) 0));
        return true;
    }*/
    ...
    public static void SunziBF(final LittleEndianAccessor slea, final MapleClient c) {
        ...
        slea.readInt();
        byte slot = (byte) slea.readShort();//道具在背包的格子位置
        int itemid = slea.readInt();//道具的ID
        MapleItemInformationProvider ii = MapleItemInformationProvider.getInstance();
        Item item = c.getPlayer().getInventory(MapleInventoryType.USE).getItem(slot);
        ...
        MapleInventoryManipulator.removeFromSlot(c, MapleInventoryType.USE, slot, (short) 1, false);
        c.getPlayer().dropMessage(5, "恭喜你从 " + ii.getName(itemid) + " 中获得了" + expGained + "经验值。");
        c.sendPacket(CWvsContext.enableActions());
    }

    public static void UseItemBaowu(LittleEndianAccessor slea, MapleClient c) {
        ...
        switch (item) {
            case 2022336://新手礼包箱子
                c.getPlayer().gainItem(2000005, 20);
                c.getPlayer().gainMeso(100000, true);
                c.getPlayer().removeAll(2022336);
                c.getPlayer().dropMessage(5, "恭喜你领取新手礼包。获得超级药水x20个。金币x10万");
                break;
            default:
                if (c.getPlayer().isGM()) {
                    c.getPlayer().dropMessage(5, "当前使用箱子ID为：" + item + "名称：" + MapleItemInformationProvider.getInstance().getName(item));
                } else {
                    c.getPlayer().dropMessage(5, "当前使用的道具尚未处理，我们已经将这个道具从您的背包删除了~");
                    MapleInventoryManipulator.removeFromSlot(c, MapleInventoryType.USE, slot, (byte) 1, false);
                    c.sendPacket(CWvsContext.enableActions());
                }
                break;

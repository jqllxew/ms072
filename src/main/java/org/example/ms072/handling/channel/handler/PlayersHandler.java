...
import  org.example.ms072.scripting.ReactorScriptManager;
...
import  org.example.ms072.server.events.MapleCoconut;
import  org.example.ms072.server.events.MapleCoconut.MapleCoconuts;
import  org.example.ms072.server.events.MapleEventType;
...
import  org.example.ms072.server.maps.MapleDoor;
...
import  org.example.ms072.server.maps.MechDoor;
...
import  org.example.ms072.tools.packet.CWvsContext.Reward;
import  org.example.ms072.tools.packet.JobPacket;
...
//        System.err.println("onReward");
    ...
    public static void updateSpecialStat(final LittleEndianAccessor slea, final MapleClient c) {
        ...
        switch (stat) {
            case "honorLeveling":
                c.sendPacket(CWvsContext.updateSpecialStat(stat, array, mode, c.getPlayer().getHonourNextExp()));
                break;
            case "hyper":
    ...
    public static void calcHyperSkillPointCount(MapleClient c) {
        for (int i = 0; i < 3; i++) {
            c.sendPacket(CWvsContext.updateHyperSp(i, c.getPlayer().getRemainingHSps()[i]));
        }
    }
    ...
    public static void LuckyLuckyMonstory(LittleEndianAccessor slea, MapleClient c) {
        c.sendPacket(CField.getLuckyLuckyMonstory());
    }
    ...
    public static void LieDetector(LittleEndianAccessor slea, MapleClient c, MapleCharacter chr, boolean isItem) {
        ...
        if (((search_chr.getAntiMacro().isPassed()) && (isItem)) || (search_chr.getAntiMacro().canDetector(System.currentTimeMillis()))) {
            c.sendPacket(CWvsContext.LieDetectorResponse((byte) 2));//已经使用过测谎仪
            c.sendPacket(CWvsContext.enableActions());
            return;
        }
    ...
    public static void LieDetectorResponse(LittleEndianAccessor slea, MapleClient c) {
        ...
        if (answer.equalsIgnoreCase(ld.getAnswer())) {
            ...
            c.sendPacket(CWvsContext.LieDetectorResponse((byte) 9, (byte) 0));
            ...
            ld.end();
            c.getPlayer().getClient().sendPacket(CWvsContext.LieDetectorResponse((byte) 7, (byte) 0));

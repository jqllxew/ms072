...
import static org.example.ms072.client.BuddyList.BuddyOperation.ADDED;
import static org.example.ms072.client.BuddyList.BuddyOperation.DELETED;
...
public class BuddyListHandler {
    ...
    private static CharacterIdNameBuddyCapacity getCharacterIdAndNameFromDatabase(final String name, final String group) throws SQLException {
        ...
        try (Connection con = DatabaseConnection.getInstance().getDataSource().getConnection(); PreparedStatement ps = con.prepareStatement("SELECT * FROM characters WHERE name LIKE ?")) {
            ps.setString(1, name);
    ...
    public static final void BuddyOperation(final LittleEndianAccessor slea, final MapleClient c) {
        ...
        if (mode == 1) { // add
            ...
            if (ble != null && (ble.getGroup().equals(groupName) || !ble.isVisible())) {
                c.sendPacket(BuddylistPacket.buddylistMessage((byte) 0x0B));
            } else if (ble != null && ble.isVisible()) {
                ble.setGroup(groupName);
                c.sendPacket(BuddylistPacket.updateBuddylist(buddylist.getBuddies(), 0x07));
                c.sendPacket(BuddylistPacket.buddylistMessage((byte) 0x0D));
                ...
                try {
                    ...
                    if (charWithId != null) {
                        ...
                        if (channel > 0) {
                            ...
                            try (Connection con = DatabaseConnection.getInstance().getDataSource().getConnection(); PreparedStatement ps = con.prepareStatement("SELECT COUNT(*) as buddyCount FROM buddies WHERE characterid = ? AND pending = 0")) {
                        ...
                        if (buddyAddResult == BuddyAddResult.BUDDYLIST_FULL) {
                            c.sendPacket(BuddylistPacket.buddylistMessage((byte) 0x0C));//12
                        } else {
                            ...
                            if (buddyAddResult == BuddyAddResult.ALREADY_ON_LIST && channel > 0) {
                                ...
                                try (Connection con = DatabaseConnection.getInstance().getDataSource().getConnection();PreparedStatement ps = con.prepareStatement("INSERT INTO buddies (`characterid`, `buddyid`, `groupname`, `pending`) VALUES (?, ?, ?, 1)")) {
                                    ps.setInt(1, charWithId.getId());
                        ...
                        c.sendPacket(BuddylistPacket.buddylistMessage((byte) 0x0F));//was15
                    }
                } catch (SQLException e) {
            ...
            if (!buddylist.isFull() && ble != null && !ble.isVisible()) {
                ...
                c.sendPacket(BuddylistPacket.buddylistMessage((byte) 0x0B));//11
            }

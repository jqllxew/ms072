...
public class HiredMerchantHandler {
    ...
    public static final boolean UseHiredMerchant(final MapleClient c, final boolean packet) {
        if (World.isShutDown) {
            c.getPlayer().dropMessage(1, "服务器正在关闭，现在无法开店！\r\n请立即下线，否则后果自负！");
    ...
    public static void displayMerch(MapleClient c) {
        ...
        if (merch) {
            ...
            if (pack == null) {
                ...
                c.sendPacket(NPCPacket.getNPCTalk(9030000, (byte) 0, "您雇佣商店保存的道具已发送到背包上.", "00 00", (byte) 0));
                c.getPlayer().setConversation(0);
            }
        }
        c.sendPacket(CWvsContext.enableActions());
    ...
    public static final void MerchantItemStore(final LittleEndianAccessor slea, final MapleClient c) {
        ...
        if (World.isShutDown) {
            c.getPlayer().dropMessage(1, "服务器正在关闭，现在无法取回道具！\r\n请立即下线，否则后果自负！");
            c.sendPacket(CWvsContext.enableActions());
            return;
        }
...
//            }
        ...
        switch (operation) {
            ...
            case 25: { // 要求拿出物品
                ...
                c.sendPacket(PlayerShopPacket.merchItemStore((byte) 0x24, 0, 0));// 38
                break;
            }
            case 26: { // 取出物品
                if (c.getPlayer().getConversation() != 3) {
                ...
                if (deletePackage(c.getPlayer().getAccountID(), pack.getPackageid(), c.getPlayer().getId())) {
                    ...
                    c.getPlayer().setConversation(0);
                    FileoutputUtil.logToFile_chr(c.getPlayer(), "logs/Data/Log_雇佣领取记录.txt", " 领回金币 " + pack.getMesos() + " 领回道具数量 " + pack.getItems().size() + " 道具 " + item_id);
                    FileoutputUtil.logToFile_chr(c.getPlayer(), "logs/Data/Log_雇佣领取记录2.txt", " 领回金币 " + pack.getMesos() + " 领回道具数量 " + pack.getItems().size() + " 道具 " + item_name);
                    //    c.getPlayer().setLastHM(System.currentTimeMillis());
                } else {
                    c.getPlayer().dropMessage(1, "发生未知错误.");
                }

...
public class BeanGame {
    ...
    public static final void BeansGameAction(LittleEndianAccessor slea, MapleClient c) {
        ...
        switch (type) {
            ...
            case 3: { // 鋼珠進入蘑菇中獎區
                ...
                c.sendPacket(CField.showBeansTips(new String[]{chr.getName()}));
                break;
            }
            case 6: { // 發射鋼珠訊息
                ...
                c.sendPacket(CField.shootBeans(unk, beansInfo));
                break;
            }
            default: {
    ...
    public static final void BeansUpdate(LittleEndianAccessor slea, MapleClient c) {
        //c.sendPacket(CField.updateBeans(c.getPlayer().getId(), c.getPlayer().getBeans()));

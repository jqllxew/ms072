...
import  org.example.ms072.scripting.LieDetectorScript;
...
public class MapleLieDetector {
    ...
    public final boolean startLieDetector(final String tester, final boolean isItem, final boolean anotherAttempt) {
        ...
        attempt -= 1;
        this.chr.getClient().sendPacket(CWvsContext.sendLieDetector(image, attempt + 1));
            ...
            public void run() {
                if ((!MapleLieDetector.this.isPassed()) && (MapleLieDetector.this.chr != null)) {
                    if (MapleLieDetector.this.attempt == -1) {
                        ...
                        MapleLieDetector.this.chr.getClient().sendPacket(CWvsContext.LieDetectorResponse((byte) 7, (byte) 0));

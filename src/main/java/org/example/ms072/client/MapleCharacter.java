...
import org.example.ms072.client.MapleTrait.MapleTraitType;
import org.example.ms072.client.anticheat.CheatTracker;
import org.example.ms072.client.anticheat.ReportType;
import org.example.ms072.client.inventory.*;
import org.example.ms072.client.inventory.MapleImp.ImpFlag;
...
import org.example.ms072.custom.MoonlightAchievements;
...
import org.example.ms072.handling.login.LoginInformationProvider.JobType;
...
import org.example.ms072.handling.world.guild.MapleGuildCharacter;
import org.example.ms072.scripting.EventInstanceManager;
import org.example.ms072.scripting.EventManager;
...
import org.example.ms072.server.Timer.MapTimer;
import org.example.ms072.server.Timer.WorldTimer;
...
import org.example.ms072.server.shops.MapleShopItem;
...
import org.example.ms072.tools.packet.*;
import org.example.ms072.tools.packet.CField.EffectPacket;
import org.example.ms072.tools.packet.CField.SummonPacket;
import org.example.ms072.tools.packet.CWvsContext.*;
import org.example.ms072.tools.packet.JobPacket.AvengerPacket;
import org.example.ms072.tools.packet.JobPacket.LuminousPacket;
import org.example.ms072.tools.packet.JobPacket.PhantomPacket;
import org.example.ms072.tools.packet.JobPacket.XenonPacket;
...
public class MapleCharacter extends AnimatedMapleMapObject implements Serializable, MapleCharacterLook {
    ...
    private int accountid, id, hair, face, secondHair, secondFace, faceMarking, ears, tail, elf, mapid, fame, pvpExp, pvpPoints, totalWins, totalLosses,
            ...
            rank = 1, rankMove = 0, jobRank = 1, jobRankMove = 0, marriageId, marriageItemId, dotHP, vip = 0,
    ...
    public static MapleCharacter getDefault(final MapleClient client, final JobType type) {
        ...
        try (Connection con = DatabaseConnection.getInstance().getDataSource().getConnection(); PreparedStatement ps = con.prepareStatement("SELECT * FROM accounts WHERE id = ?");) {
    ...
    public static MapleCharacter ReconstructChr(final CharacterTransfer ct, final MapleClient client, final boolean isChannel) {
        ...
        for (final Entry<Byte, Integer> qs : ct.reports.entrySet()) {
        ...
        try (Connection con = DatabaseConnection.getInstance().getDataSource().getConnection()) {
            ...
            try {
                ...
                ps = con.prepareStatement("INSERT INTO characters " +
                        "(level, str, dex, luk, `int`, hp, mp, maxhp, maxmp, sp, hsp, ap, skincolor, gender, job, hair, face, faceMarking, ears, tail, map, meso, party, buddyCapacity, pets, subcategory, elf, friendshippoints, chatcolour, gm, accountid, name, world, beans, megatimes)"
                        + "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
...
//            }
    ...
    public final void updateQuest(final MapleQuestStatus quest, final boolean update) {
        ...
        if (!(quest.isCustom())) {
            ...
            if (quest.getStatus() == 1 && !update) {
                client.sendPacket(CField.updateQuestInfo(this, quest.getQuest().getId(), quest.getNpc(), (byte) 0x8));//was10
            }
        }
    }
    ...
    public void doDragonBlood() {
        ...
        if (stats.getHp() - bloodEffect.getX() <= 1) {
            ...
            client.sendPacket(EffectPacket.showOwnBuffEffect(bloodEffect.getSourceId(), 5, getLevel(), bloodEffect.getLevel()));
        ...
        while (!passed) {
            ...
            switch (randval) {
                case 0: // Meso
                    ...
                    gainMeso(money, true);
                    client.sendPacket(CWvsContext.fishingUpdate((byte) 1, money));
                    passed = true;
                    break;
                case 1: // EXP
                    ...
                    client.sendPacket(CWvsContext.fishingUpdate((byte) 2, experi));
                    passed = true;
                    break;
                default:
                    if (MapleItemInformationProvider.getInstance().itemExists(randval)) {
                        ...
                        client.sendPacket(CWvsContext.fishingUpdate((byte) 0, randval));
                        passed = true;
                    }
                    break;
            }
...
//        }
            ...
            if (targets > 0) {
                if (getBuffedValue(MapleBuffStat.ENERGY_CHARGE) == null) {
                    ...
                    if (energyLevel < 10000) {
                        ...
                        this.client.sendPacket(EffectPacket.showOwnBuffEffect(skillid, 2, getLevel(), skilllevel));
                        this.map.broadcastMessage(this, EffectPacket.showBuffeffect(this.id, skillid, 2, getLevel(), skilllevel), false);

                        if (energyLevel >= 10000) {
                            energyLevel = 10000;
                        }
                        this.client.sendPacket(BuffPacket.giveEnergyChargeTest(energyLevel, echeff.getDuration() / 1000));
    ...
    public final void handleOrbgain() {
        ...
        if (orbcount < ceffect.getX() + 1) {
            ...
            this.client.sendPacket(BuffPacket.giveBuff(combo.getId(), duration, stat, ceffect));
...
//        System.out.println(start + " | " + end);
    ...
    public void gainExpMonster(final int gain, final boolean show, final boolean white, final byte pty, int Class_Bonus_EXP, int Equipment_Bonus_EXP, int Premium_Bonus_EXP, boolean partyBonusMob, final int partyBonusRate) {
        ...
        if (gain != 0) {
            ...
            if (show) { // still show the expgain even if it's not there
                client.sendPacket(InfoPacket.GainEXP_Monster(expM, white, partyinc, Class_Bonus_EXP, Equipment_Bonus_EXP, Premium_Bonus_EXP, Marriage_Bonus_EXP, Team_Bonus_Rate, Activity_Bonus_EXP));
            }
        }
    }
...
//            client.sendPacket(InventoryPacket.updateSpecialItemUse(item, type == MapleInventoryType.EQUIPPED ? (byte) 1 : type.getType(), this));
        ...
        if (pending) {
            ...
            if (pendingSkills != null) {
                ...
                for (Skill z : pendingSkills.keySet()) {
                    client.sendPacket(CWvsContext.broadcastMsg(5, "[" + SkillFactory.getSkillName(z.getId()) + "] 技能已过期，将无法使用."));
                }
            } //not real msg
            pendingSkills = null;
            return;
        ...
        if (sendPacket && getCooldowns() != null) {
            for (MapleCoolDownValueHolder cooldown : getCooldowns()) {
                ...
                if (lefttime > 0) {
                    ...
                    getClient().sendPacket(CField.skillCooldown(cooldown.skillId, lefttime));
                }
            }
        }
    }
    ...
    public void giveDebuff(MapleDisease disease, int x, long duration, int skillid, int level) {
        if ((this.map != null) && (!hasDisease(disease))) {
            ...
            this.client.sendPacket(BuffPacket.giveDebuff(disease, x, skillid, level, (int) duration));
    ...
    public void dropMessage(int type, String message) {
        switch (type) {
            ...
            case -9:
                client.sendPacket(InfoPacket.showInfo(message));
            case -10:
                client.sendPacket(CField.getFollowMessage(message));
            case -11:
                this.client.sendPacket(CWvsContext.yellowChat(message));
            default:
                ...
                if (pet != null && (item.getItemId() != 5000054 || pet.getSecondsLeft() > 0) && (item.getExpiration() == -1 || item.getExpiration() > System.currentTimeMillis())) {
                    if (pet.getSummoned()) { // Already summoned, let's keep it
                        ...
                        if (/*broadcast &&*/getMap() != null) {
                            //client.sendPacket(PetPacket.updatePet(pet, getInventory(MapleInventoryType.CASH).getItem((short) (byte) pet.getInventoryPosition()), pet.getSummoned()));
    ...
    public void changeChannel(int channel) {
        ...
        client.sendPacket(CField.getChannelChange(client, toch.getPort()));
    ...
    public boolean hasBlockedInventory() {
        // System.out.println("getConversation() ："+getConversation());
...
//        updateSingleStat(MapleStat.FATIGUE, this.fatigue);
    ...
    public final String getReportSummary() {
        ...
        ret = offenseList.stream().map(reportTypeIntegerPair -> StringUtil.makeEnumHumanReadable(reportTypeIntegerPair.left.name()) + ": " + reportTypeIntegerPair.right + " ").collect(Collectors.joining());
        return ret;
    }
    ...
    public void afterAttack(int mobCount, int attackCount, int skillid) {
        ...
        if (!isIntern()) {
            ...
            if (ds != null) {
                if (ds.getSourceId() != 4221007 || !ds.makeChanceResult() || ds.getSourceId() == 0) {
...
//    }
    ...
    public void sendWelcome() {
        getClient().sendPacket(CField.startMapEffect("欢迎光临 " + getClient().getChannelServer().getServerName() + "!", 5122000, true));
    ...
    public void setVote() {
        ...
        try (Connection con1 = DatabaseConnection.getInstance().getDataSource().getConnection()) {
            try (PreparedStatement ps = con1.prepareStatement("INSERT INTO ipvotes (`ip`, `accid`, `lastvote`) VALUES (?, ?, ?)")) {
                ps.setString(1, getClient().getSession().remoteAddress().toString().split(":")[0].replaceAll("/", ""));
    ...
    /*public void toggleCustomBGState() {
        ...
        for (byte i = 0; i < 127; i++) {
            WriteFuture write = client.sendPacket(CField.removeBGLayer((getCustomBGState() == 1), 0, i, 0));
            //duration 0 = forever map 0 = current map
        }
    }*/
    ...
    public void changeLuminousMode(final int skillid) {
        ...
        client.sendPacket(LuminousPacket.giveLuminousState(20040220, chr.getLightGauge(), chr.getDarkGauge(), 0));
        ...
        WorldTimer.getInstance().schedule(new Runnable() {
            ...
            public void run() {
                dispelBuff(20040220);
                client.sendPacket(LuminousPacket.giveLuminousState(skillid, chr.getLightGauge(), chr.getDarkGauge(), 0));
    ...
    public void changeWarriorStance(final int skillid) {
        if (skillid == 11101022) {
             ...
             * client.sendPacket(DawnWarriorPacket.giveEquinox_Moon(getSkillLevel(skillid),
             * Integer.MAX_VALUE));
             * SkillFactory.getSkill(skillid).getEffect(1).applyTo(this); } else
             * if (skillid == 11121012) { dispelBuff(11101022);
             * client.sendPacket(DawnWarriorPacket.giveEquinox_Sun(getSkillLevel(skillid),
             * Integer.MAX_VALUE));
             * SkillFactory.getSkill(skillid).getEffect(1).applyTo(this);
             */
        }
...
//        client.sendPacket(EffectPacket.updateDeathCount(deathCount));
    ...
    public void updateSkills() {
        ...
        client.sendPacket(CField.getKeymap(getKeyLayout()));
    ...
    public void sendEnglishQuiz(String msg) {
        client.sendPacket(CWvsContext.englishQuizMsg(msg));
    }

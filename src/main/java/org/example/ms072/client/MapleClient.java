...
import org.example.ms072.handling.netty.MaplePacketDecoder;
...
import org.example.ms072.server.CharacterCardFactory;
...
import org.example.ms072.server.shark.SharkLogger;
...
public class MapleClient implements Serializable {
    ...
    public final void sendPacket(final byte[] array) {
    ...
    private Calendar getTempBanCalendar(ResultSet rs) throws SQLException {
        ...
        if (tempban == null) { // basically if timestamp in db is 0000-00-00
    ...
    public final void setGender(final byte gender) {
        this.gender = gender;
        try (Connection con = DatabaseConnection.getInstance().getDataSource().getConnection(); PreparedStatement ps = con.prepareStatement("UPDATE accounts SET gender = ? WHERE id = ?")) {
            ps.setByte(1, gender);
        ...
        PingTimer.getInstance().schedule(new Runnable() {
            ...
            public void run() {
                try {
                    ...
                    System.err.println("sendPing NullPointerException : " + e.toString());
    ...
    public final void unLockDisconnect() {
        sendPacket(CWvsContext.serverNotice(1, "当前账号已在别处登录\r\n若不是您本人操作请及时更改密码"));
    ...
    public void updateMacsS(String macData) {
        if (macData.equalsIgnoreCase("00-00-00-00-00-00") || macData.length() != 17) {
            return;
        }
        try {

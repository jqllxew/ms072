package org.example.ms072.server.quest;
...
import  org.example.ms072.client.MapleTrait;
...
import  org.example.ms072.client.inventory.InventoryException;
...
public class MapleQuestAction
    ...
    public void runStart(MapleCharacter c, Integer extSelection) {
        ...
        switch (type) {
            ...
            case nextQuest:
                status = c.getQuest(this.quest);
                if (status.getForfeited() <= 0) {
                    c.getClient().sendPacket(CField.updateQuestFinish(this.quest.getId(), status.getNpc(), this.intStore));
                }
                break;
            case money:
                status = c.getQuest(this.quest);
    ...
    public void runEnd(MapleCharacter c, Integer extSelection) {
        ...
        switch (type) {
            ...
            case nextQuest:
                c.getClient().sendPacket(CField.updateQuestFinish(this.quest.getId(), c.getQuest(this.quest).getNpc(), this.intStore));
                break;
            case money:
                c.gainMeso(this.intStore, true, true, false);
                break;

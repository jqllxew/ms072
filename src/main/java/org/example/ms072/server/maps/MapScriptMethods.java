...
import org.example.ms072.server.life.OverrideMonsterStats;
import org.example.ms072.server.maps.MapleNodes.DirectionInfo;
import org.example.ms072.server.quest.MapleQuest;
import org.example.ms072.server.quest.MapleQuest.MedalQuest;
...
//                int ad = Integer.parseInt(lol);
    ...
    public static void startScript_User(final MapleClient c, String scriptName) {
        ...
        switch (onUserEnter.fromString(scriptName)) {
             ...
             * c.sendPacket(UIPacket.getDirectionInfo(3, 0));
             * c.sendPacket(CField.UIPacket.IntroDisableUI(true)); if
             * (!c.getPlayer().getMap().containsNPC(9390381)) {
             * c.getPlayer().getMap().spawnNpc(9390381, new Point(89, 32)); }
             * c.sendPacket(NPCPacket.setNPCSpecialAction(9390381,
             * "summon")); c.sendPacket(UIPacket.getDirectionInfo(1,
             ...
             * c.sendPacket(UIPacket.IntroEnableUI(0));
             * c.sendPacket(UIPacket.IntroDisableUI(false));
             * c.sendPacket(CField.sendHint("Press
             ...
             * c.sendPacket(CWvsContext.getTopMsg("Earned Forever Single
            ...
            case Advanture_tuto00: {
                try {
                    ...
                    c.sendPacket(UIPacket.getDirectionInfo(1, 2100));
                    Thread.sleep(2800);
                    c.sendPacket(CWvsContext.getTopMsg("按左右键移动."));
                    c.sendPacket(UIPacket.getDirectionInfo(1, 3000));
                    Thread.sleep(3000);
                    c.sendPacket(CWvsContext.getTopMsg("转到门户所在的地方，并按上键移动到下一个地图."));
                } catch (InterruptedException e) {
                }
                c.sendPacket(UIPacket.IntroEnableUI(0));
                break;
            ...
            case Advanture_tuto01: {
                try {
                    ...
                    c.sendPacket(UIPacket.getDirectionInfo(1, 1800));
                    Thread.sleep(1800);
                    c.sendPacket(CWvsContext.getTopMsg("按Ctrl键使用普通攻击."));
                } catch (InterruptedException e) {
                }
                c.sendPacket(UIPacket.IntroEnableUI(0));
                break;
            ...
            case Advanture_tuto10: {
                try {
                    ...
                    c.sendPacket(CWvsContext.getTopMsg("按向下键和Alt键."));
                } catch (InterruptedException e) {
                }
                c.sendPacket(UIPacket.IntroEnableUI(0));
...
//                    c.sendPacket(UIPacket.getUnknownDirectionInfo("Effect/Direction12.img/effect/tuto/BalloonMsg1/1", 900, 0, -120, 814124));//improve
//                    c.sendPacket(UIPacket.getDirectionInfo(1, 1800));
//                    Thread.sleep(1800);
//                    c.sendPacket(NPCPacket.NPCSpecialAction(814124, 1, 1000));
//                    c.sendPacket(UIPacket.getDirectionInfoNew((byte) 0, 200));
//                    c.sendPacket(UIPacket.getDirectionInfo(1, 4542));
//                    Thread.sleep(4542);
//                    c.sendPacket(UIPacket.getDirectionInfoNew((byte) 1, 300));
//                    c.sendPacket(UIPacket.getDirectionInfo(1, 2834));
//                    Thread.sleep(2834);
//                    c.sendPacket(UIPacket.getDirectionInfo(1, 900));
//                    Thread.sleep(900);
                //npc
...
//                }
            ...
            case enter_101070000: {
                try {
                    c.sendPacket(CWvsContext.getTopMsg("仙女的森林似乎在你退出通道的地方实现."));
                    Thread.sleep(2000);
                } catch (InterruptedException e) {
                }
            ...
            case check_q20833:
                if (c.getPlayer().getQuestStatus(20833) == 1) {
                    ...
                    c.sendPacket(CWvsContext.getTopMsg("地图右边的那个人是谁?"));
                }
                break;
            case q2614M:
...
//                c.getPlayer().setDirection(0);
            ...
            case explorationPoint: {
                ...
                if (m != null && c.getPlayer().getLevel() >= m.level && c.getPlayer().getQuestStatus(m.questid) != 2) {
                    ...
                    if (changedd) {
                        ...
                        c.sendPacket(CWvsContext.showQuestMsg("标题 " + String.valueOf(m) + " 正在进行中的探险家 " + number + "/" + m.maps.length + " 完整的"));
                    }
                }
                break;
            }
            ...
            case dubl2Tuto10: {
                c.sendPacket(CWvsContext.getTopMsg("秘密花园深处"));
                c.sendPacket(CWvsContext.getTopMsg("在一个下雨天..."));
                break;
            }
    ...
    private static void sendDojoStart(MapleClient c, int stage) {
        ...
        c.sendPacket(CField.environmentChange("Dojang/start", 4));//was4
        c.sendPacket(CField.environmentChange("dojang/start/stage", 3));//was3
        c.sendPacket(CField.environmentChange("dojang/start/number/" + stage, 3));//was3

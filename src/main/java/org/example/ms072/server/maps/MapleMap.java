...
import org.example.ms072.client.MonsterStatusEffect;
...
import org.example.ms072.server.MapleSquad.MapleSquadType;
...
import org.example.ms072.server.Timer.EtcTimer;
import org.example.ms072.server.Timer.MapTimer;
import org.example.ms072.server.events.MapleEvent;
import org.example.ms072.server.life.MapleLifeFactory;
import org.example.ms072.server.life.MapleMonster;
import org.example.ms072.server.life.MapleMonsterInformationProvider;
import org.example.ms072.server.life.MapleNPC;
import org.example.ms072.server.life.MonsterDropEntry;
import org.example.ms072.server.life.MonsterGlobalDropEntry;
import org.example.ms072.server.life.SpawnPoint;
import org.example.ms072.server.life.SpawnPointAreaBoss;
import org.example.ms072.server.life.Spawns;
import org.example.ms072.server.maps.MapleNodes.DirectionInfo;
import org.example.ms072.server.maps.MapleNodes.MapleNodeInfo;
import org.example.ms072.server.maps.MapleNodes.MaplePlatform;
import org.example.ms072.server.maps.MapleNodes.MonsterPoint;
...
import org.example.ms072.tools.packet.CWvsContext.PartyPacket;
                ...
                for (final MapleCharacter mc : characters) {
                    if (mc.isAlive()) {
                        ...
                        switch (monster.getId()) {
                            ...
                            case 8810122:
                            case 8820001:
                                mc.getClient().sendPacket(EffectPacket.showOwnBuffEffect(buffid, 11, mc.getLevel(), 1)); // HT nine spirit
...
//            FileoutputUtil.log(FileoutputUtil.Zakum_Log, MapDebug_Log());
     ...
     *
     * public final void sendPackets(MapleClient c) {
     * c.sendPacket(MobPacket.spawnMonster(monster,
        ...
        spawnAndAddRangedMapObject(summon, new DelayedPacketCreation() {
            ...
            public void sendPackets(MapleClient c) {
                if (summon != null && c.getPlayer() != null && (!summon.isChangedMap() || summon.getOwnerId() == c.getPlayer().getId())) {
                    ...
                    c.sendPacket(SummonPacket.spawnSummon(summon, true));// 079 这样才会显示
                }
            }
        });
    }
...
//	broadcastMessage(CField.dropItemFromMapObject(mdrop, mob.getTruePosition(), dropPos, (byte) 0));
    ...
    public final void addPlayer(final MapleCharacter chr) {
        ...
        if (hasBoat() == 2) {
            chr.getClient().sendPacket(CField.boatPacket(true));
        } else if (hasBoat() == 1 && (chr.getMapId() != 200090000 || chr.getMapId() != 200090010)) {
            chr.getClient().sendPacket(CField.boatPacket(false));
        }
        if (!chr.isClone()) {
        ...
        for (final MaplePet pet : chr.getSummonedPets()) {
            if (pet.getSummoned()) {
                ...
                // chr.getClient().sendPacket(PetPacket.updatePet(pet, chr.getInventory(MapleInventoryType.CASH).getItem((short) (byte) pet.getInventoryPosition()), pet.getSummoned()));
         ...
         * chr.getClient().sendPacket(CField.getQuickMoveInfo(true,
         * qmn)); break; } } } else {
         * chr.getClient().sendPacket(CField.getQuickMoveInfo(false, new
         * LinkedList())); }
         */

        if (getNPCById(9073000) != null && getId() == 931050410) {
            chr.getClient().sendPacket(NPCPacket.toggleNPCShow(getNPCById(9073000).getObjectId(), true));
        }
        ...
        if (!chr.isClone()) {
            if (chr.getChalkboard() != null) {
                chr.getClient().sendPacket(CSPacket.useChalkboard(chr.getId(), chr.getChalkboard()));
            }
    ...
    public void getRankAndAdd(String leader, String time, ExpeditionType type, long timz, Collection<String> squad) {
        try {
            ...
            try (Connection con = DatabaseConnection.getInstance().getDataSource().getConnection(); PreparedStatement ps = con.prepareStatement("INSERT INTO speedruns(`type`, `leader`, `timestring`, `time`, `members`) VALUES (?,?,?,?,?)")) {

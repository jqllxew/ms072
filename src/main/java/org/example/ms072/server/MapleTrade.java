...
import org.example.ms072.server.commands.CommandProcessor;
...
public class MapleTrade {
    ...
    public final void CompleteTrade() {
        ...
        this.chr.get().getClient().sendPacket(CField.InteractionPacket.TradeMessage(this.tradingslot, (byte) 8));
    }
    ...
    public final void setMeso(int meso) {
        ...
        if (this.chr.get().getMeso() >= meso) {
            ...
            this.meso += meso;
            this.chr.get().getClient().sendPacket(CField.InteractionPacket.getTradeMesoSet((byte) 0, this.meso));
    ...
    public final void addItem(Item item) {
        ...
        this.items.add(item);
        this.chr.get().getClient().sendPacket(CField.InteractionPacket.getTradeItemAdd((byte) 0, item));
        if (this.partner != null) {
    ...
    public final void chat(String message) throws Exception {
        if (!CommandProcessor.processCommand(chr.get().getClient(), message, ServerConstants.CommandType.TRADE)) {
            ...
            if (this.partner != null) {
                this.partner.getChr().getClient().sendPacket(PlayerShopPacket.shopChat(this.chr.get().getName() + " : " + message, 1));
            }
        }
        ...
        if (this.exchangeItems != null) {
            ...
            for (Item item : this.exchangeItems) {
                switch (GameConstants.getInventoryType(item.getItemId())) {
                    case EQUIP -> eq = (byte) (eq + 1);
                    case USE -> use = (byte) (use + 1);
                    case SETUP -> setup = (byte) (setup + 1);
                    case ETC -> etc = (byte) (etc + 1);
                    case CASH -> cash = (byte) (cash + 1);
                }
    ...
    public static final void startTrade(MapleCharacter c) {
        if (World.isShutDown) {
            c.getClient().sendPacket(CWvsContext.serverNotice(5, "服务器正在关闭，现在无法交易！\r\n请立即下线，否则后果自负！"));
            return;
        }
        if (c.getTrade() == null) {
    ...
    public static final void inviteTrade(MapleCharacter c1, MapleCharacter c2) {
        if (World.isShutDown) {
            c1.getTrade().cancel(c1.getClient(), c1, 1);
            c1.setTrade(null);
            c1.getClient().sendPacket(CWvsContext.serverNotice(5, "服务器正在关闭，现在无法交易！\r\n请立即下线，否则后果自负！"));
            return;
        }
    ...
    public static final void visitTrade(MapleCharacter c1, MapleCharacter c2) {
        if ((c2 != null) && (c1.getTrade() != null) && (c1.getTrade().getPartner() == c2.getTrade()) && (c2.getTrade() != null) && (c2.getTrade().getPartner() == c1.getTrade())) {
            ...
            c1.getClient().sendPacket(CWvsContext.enableActions());
            c2.getClient().sendPacket(CWvsContext.enableActions());
        } else {
            c1.getClient().sendPacket(CWvsContext.broadcastMsg(5, "对方已经取消了交易。"));
        }
    }

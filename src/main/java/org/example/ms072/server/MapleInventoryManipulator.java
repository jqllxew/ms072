...
public class MapleInventoryManipulator {
    ...
    public static void addRing(MapleCharacter chr, int itemId, int ringId, int sn, String partner) {
        ...
        // chr.getClient().sendPacket(CSPacket.sendBoughtRings(InventoryConstants.isCrushRing(itemId), ring, sn, chr.getClient().getAccID(), partner));
    }
    ...
    public static byte addId(MapleClient c, int itemId, short quantity, String owner, MaplePet pet, long period, boolean hours, String gmLog) {
        ...
        if (itemId >= 4031332 && itemId <= 4031341) {
            c.sendPacket(CField.getGameMessage("Hint: Use @event to exchange a certificate of straight wins.", (short) 8));
        }
    ...
    public static boolean addFromDrop(final MapleClient c, Item item, final boolean show, final boolean enhance, final boolean isPetPickup) {
        if (World.isShutDown) {
            c.getPlayer().dropMessage(5, "服务器正在关闭，现在无法捡取物品！\r\n请立即下线，否则后果自负！");
            c.sendPacket(InventoryPacket.getInventoryFull());
            c.sendPacket(InventoryPacket.showItemUnavailable());
            return false;
        }
                    ...
                    while (quantity > 0) {
                        if (i.hasNext()) {
                            ...
                            if (oldQ < slotMax && item.getOwner().equals(eItem.getOwner()) && item.getExpiration() == eItem.getExpiration()) {
                                ...
                                eItem.setQuantity(newQ);
                                if (isPetPickup) {
                                    c.sendPacket(InventoryPacket.addInventorySlot(type, eItem, false));
                                } else {
                                    c.sendPacket(InventoryPacket.addInventorySlot(type, eItem, true));
                                }
                            }
                        } else {
                            break;
    ...
    public static boolean removeFromSlot(final MapleClient c, final MapleInventoryType type, final short slot, short quantity, final boolean fromDrop, final boolean consume) {
        ...
        if (item != null) {
            ...
            if (item.getQuantity() == 0 && !allowZero) {
                ...
                c.sendPacket(InventoryPacket.updateInventorySlot(type, item, fromDrop));
            }
            return true;
        }
        return false;
    ...
    public static void move(final MapleClient c, final MapleInventoryType type, final short src, final short dst) {
        ...
        if (!type.equals(MapleInventoryType.EQUIP) && initialTarget != null
            ...
            if ((olddstQ + oldsrcQ) > slotMax) {
                ...
                c.sendPacket(InventoryPacket.moveAndMergeInventoryItem(type, src, dst, c.getPlayer().getInventory(type).getItem(dst).getQuantity(), bag, switchSrcDst, bothBag));
            }
        } else {
    ...
    public static boolean drop(final MapleClient c, MapleInventoryType type, final short src, short quantity, final boolean npcInduced) {
        if (World.isShutDown) {
            c.getPlayer().dropMessage(5, "服务器正在关闭，现在无法丢弃物品！\r\n请立即下线，否则后果自负！");
            c.sendPacket(CWvsContext.enableActions());
            return false;
        }
        final MapleItemInformationProvider ii = MapleItemInformationProvider.getInstance();
    ...
    public static void removeAllByEquipOnlyId(MapleClient c, long inventoryitemid) {
        ...
        if (copyEquipItems != null) {
            ...
            FileoutputUtil.log("logs/Hack/复制装备_已删除.txt", msgtext + " 道具唯一ID: " + copyEquipItems.getEquipOnlyId());
        }
        ...
        if (copyEquipedItems != null) {
            ...
            World.Broadcast.broadcastGMMessage(CWvsContext.serverNotice(6, "[GM密语] " + msgtext));
            FileoutputUtil.logToFile("logs/Hack/复制装备_已删除.txt", msgtext + " 道具唯一ID: " + copyEquipedItems.getEquipOnlyId());
        }
        ...
        for (Item copyStorageItem : c.getPlayer().getStorage().getItems()) {
            if (copyStorageItem != null) {
                if (c.getPlayer().getStorage().removeItemByInventoryItemId(inventoryitemid)) {
                    ...
                    FileoutputUtil.logToFile("logs/Hack/复制装备_已删除.txt", msgtext + " 道具唯一ID: " + copyStorageItem.getEquipOnlyId() + "\r\n");
                }
            }
        }
    }

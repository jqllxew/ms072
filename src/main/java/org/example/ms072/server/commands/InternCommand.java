...
import  org.example.ms072.client.MapleJob;
...
import  org.example.ms072.client.inventory.OnlyID;
...
import  org.example.ms072.handling.world.CheaterData;
...
import org.example.ms072.server.ItemInformation;
        ...
        public int execute(MapleClient c, String[] splitted) {
            ...
            for (MapleMapObject monstermo : map.getMapObjectsInRange(c.getPlayer().getPosition(), range, Arrays.asList(MapleMapObjectType.MONSTER))) {
                ...
                if (!mob.getStats().isBoss() || mob.getStats().isPartyBonus() || c.getPlayer().isGM()) {
                    ...
                    mob.giveExpToCharacter(c.getPlayer(), mob.getMobExp(), false, (byte) 0, (byte) 0, (byte) 0, (byte) 0, 0);//杀怪正常获得经验
        ...
        public int execute(MapleClient c, String[] splitted) {
            if (c.getChannelServer().manualEvent(c.getPlayer())) {
                for (MapleCharacter chrs : c.getChannelServer().getPlayerStorage().getAllCharacters()) {
                    ...
                    chrs.getClient().sendPacket(CWvsContext.broadcastMsg(GameConstants.isEventMap(chrs.getMapId()) ? 0 : 26, c.getChannel(), "Event : Event Channel: " + c.getChannel() + " Event Map: " + c.getPlayer().getMap().getMapName()));
                }
            } else {
                for (MapleCharacter chrs : c.getChannelServer().getPlayerStorage().getAllCharacters()) {
    ...
    public static class 处理洗道具 extends CommandExecute {
        ...
        public void HandleOffline(MapleClient c, int chr, long inventoryitemid, long equiponlyid) {
            try (Connection con = DatabaseConnection.getInstance().getDataSource().getConnection()) {
                ...
                FileoutputUtil.logToFile("logs/Hack/复制装备_已删除.txt", msgtext + " 道具唯一ID: " + equiponlyid);

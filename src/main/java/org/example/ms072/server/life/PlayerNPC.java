...
public class PlayerNPC extends MapleNPC implements MapleCharacterLook {
    ...
    public PlayerNPC(ResultSet rs) throws Exception {
        ...
        try (Connection con = DatabaseConnection.getInstance().getDataSource().getConnection(); PreparedStatement ps = con.prepareStatement("SELECT * FROM playernpcs_equip WHERE NpcId = ?")) {
            ps.setInt(1, getId());

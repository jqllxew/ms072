...
import  org.example.ms072.constants.EventConstants;
...
import  org.example.ms072.tools.ConcurrentEnumMap;
...
public class MapleMonster extends AbstractLoadedMapleLife {
    ...
    // 角色获得经验值
    public void giveExpToCharacter(final MapleCharacter attacker, int exp, final boolean highestDamage, final int numExpSharers, byte pty, final byte Class_Bonus_EXP_PERCENT, final byte Premium_Bonus_EXP_PERCENT, final int lastskillID) {
        /*判断最高攻击*/
        if (highestDamage) {
            if (eventInstance != null) {
        ...
        if (exp > 0) {
            ...
            /*经验计算公式*/
            exp *= attacker.getEXPMod() * attacker.getStat().expBuff / 100.0;
            //exp = (int) Math.min(Integer.MAX_VALUE, exp * (attacker.getLevel() < 10 ? GameConstants.getExpRate_Below10(attacker.getJob()) : ChannelServer.getInstance(map.getChannel()).getExpRate(attacker.getWorld())));
            if (attacker.getLevel() < 10) {
                exp = Math.min(Integer.MAX_VALUE, exp * ServerConfig.LevelExp10);
            } else if (attacker.getLevel() < 30) {
                exp = Math.min(Integer.MAX_VALUE, exp * ServerConfig.LevelExp30);
            } else if (attacker.getLevel() < 70) {
                exp = Math.min(Integer.MAX_VALUE, exp * ServerConfig.LevelExp70);
            } else if (attacker.getLevel() < 120) {
                exp = Math.min(Integer.MAX_VALUE, exp * ServerConfig.LevelExp120);
            } else if (attacker.getLevel() < 150) {
                exp = Math.min(Integer.MAX_VALUE, exp * ServerConfig.LevelExp150);
            } else {
                exp = Math.min(Integer.MAX_VALUE, exp * ChannelServer.getInstance(map.getChannel()).getExpRate(attacker.getWorld()));
            }
...
//            exp = (int) Math.min(Integer.MAX_VALUE, exp * attacker.getEXPMod() * attacker.getStat().expBuff / 100.0 * (GameConstants.isTutorialMap(attacker.getMapId()) ? 1 : GameConstants.getExpRate(attacker.getJob(), ChannelServer.getInstance(map.getChannel()).getExpRate(attacker.getWorld()))));
    ...
    public final int killBy(final MapleCharacter killer, final int lastSkill) {
        ...
        if (controll != null) { // this can/should only happen when a hidden gm attacks the monster
            controll.getClient().sendPacket(MobPacket.stopControllingMonster(this));
    ...
    public final void switchController(final MapleCharacter newController, final boolean immediateAggro) {
        ...
        if (controllers == newController) {
            ...
            controllers.getClient().sendPacket(MobPacket.stopControllingMonster(this));
    ...
    public final void applyStatus(final MapleCharacter from, final MonsterStatusEffect status, final boolean poison, long duration, final boolean checkboss, final MapleStatEffect eff) {
        ...
        final BuffTimer applyEffectTimer = BuffTimer.getInstance();
    ...
    public final void cancelSingleStatus(final MonsterStatusEffect stat) {
        ...
        try {
            ...
            if (con != null) {
                ...
                con.getClient().sendPacket(MobPacket.cancelMonsterStatus(this, stat));
            } else {
